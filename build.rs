...
fn main() -> Result<(), J4rsBuildError> {
    ...
    let source_jar_location =
        PathBuf::from(format!("kosher-java/target/zmanim-2.6.0-SNAPSHOT.jar"));
...
fn copy_jars_from_java(jar_source_path: &Path) -> Result<(), J4rsBuildError> {
    if jar_source_path.exists() {
        ...
        let destination_jar_file =
            Path::new(&jassets_path).join(format!("zmanim-2.6.0-SNAPSHOT.jar"));
...
fn copy_jars_to_exec_directory(out_dir: &str) -> Result<(), J4rsBuildError> {
    ...
    let jassets_jar_file = Path::new(&jassets_path).join(format!("zmanim-2.6.0-SNAPSHOT.jar"));
    let jassets_output_file = Path::new(&jassets_output_dir)
        .join("jassets")
        .join(format!("zmanim-2.6.0-SNAPSHOT.jar"));
