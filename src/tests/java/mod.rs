...
mod java_tests {
    ...
    #[test]
    fn test_specific_sof_zman_kidush_levana_15_days() {
        let jvm = init_jvm();

        // Specific test case from error:
        // Error: None vs Some(1996-10-26T14:42:20.943MDT), get_sof_zman_kidush_levana_15_days_from_times
        // against java with calendar ZmanimCalendar { date_time: 1996-10-27T00:08:34.943MDT,
        // geo_location: GeoLocation { latitude: -70.78112004458067, longitude: 124.87485410804629,
        // elevation: 684.5681982927429 }, ...} with args Some(1996-10-26T14:42:20.943MDT),
        // Some(1996-10-27T09:44:29.943MST)

        let timezone = chrono_tz::America::Denver;

        let geo_location = GeoLocation::new(-70.78112004458067, 124.87485410804629, 684.5681982927429)
            .expect("Failed to create GeoLocation");

        let _java_geo_location = JavaGeoLocation::new(
            &jvm,
            -70.78112004458067,
            124.87485410804629,
            684.5681982927429,
        ...
        .expect("Failed to create Java GeoLocation");

        // Parse the specific datetime: 1996-10-27T00:08:34.943MDT
        let date_time = timezone
            .with_ymd_and_hms(1996, 10, 27, 0, 8, 34)
            .unwrap()
            .checked_add_signed(chrono::Duration::milliseconds(943))
            .unwrap();

        // Parse alos: 1996-10-26T14:42:20.943MDT
        let alos = Some(
            timezone
                .with_ymd_and_hms(1996, 10, 26, 14, 42, 20)
                .unwrap()
                .checked_add_signed(chrono::Duration::milliseconds(943))
                .unwrap(),
        );

        // Parse tzais: 1996-10-27T09:44:29.943MST
        let tzais = Some(
            timezone
                .with_ymd_and_hms(1996, 10, 27, 9, 44, 29)
                .unwrap()
                .checked_add_signed(chrono::Duration::milliseconds(943))
                .unwrap(),
        );

        let use_astronomical_chatzos = false;
        let use_astronomical_chatzos_for_other_zmanim = false;
        let candle_lighting_offset = chrono::Duration::minutes(49);
        let ateret_torah_sunset_offset = chrono::Duration::minutes(6);
        ...
        let rust_result = rust_calendar.get_sof_zman_kidush_levana_15_days_from_times(alos.as_ref(), tzais.as_ref());
        let java_result = java_calendar.get_sof_zman_kidush_levana_15_days_from_times(alos.as_ref(), tzais.as_ref());

        assert_almost_equal_datetime_option(
            &rust_result,
            &java_result,
            &format!(
                "get_sof_zman_kidush_levana_15_days_from_times with alos {:?}, tzais {:?}",
                alos, tzais
            ),
        );
    }
