...
mod java_tests {
    ...
    #[test]
    fn test_tchilas_zman_kidush_levana_7_days_regression() {
        // Regression test for get_tchilas_zman_kidush_levana_7_days_from_times
        // This test case was found during randomized testing and exposes a bug
        // where Rust returns None but Java returns Some(2004-03-28T02:30:40.170CDT)
        let jvm = init_jvm();
        
        // Create the specific date and location that triggers the bug
        let timezone = chrono_tz::Tz::America__Chicago;
        let date_time = timezone.with_ymd_and_hms(2004, 3, 28, 5, 34, 56).unwrap()
            + chrono::Duration::milliseconds(740);
        ...
        let rust_calendar = ZmanimCalendar::new(
            ...
            true,
            false,
            chrono::Duration::seconds(540),
            chrono::Duration::seconds(3600),
        );
        
        // Create the Java calendar
        ...
        // The specific arguments that trigger the bug
        let alos = None;
        let tzais = timezone.with_ymd_and_hms(2004, 3, 28, 23, 16, 7).unwrap()
            + chrono::Duration::milliseconds(740);
        let tzais = Some(tzais);
        
        // Call the method under test
        let rust_result = rust_calendar.get_tchilas_zman_kidush_levana_7_days_from_times(alos.as_ref(), tzais.as_ref());
        let java_result = java_calendar.get_tchilas_zman_kidush_levana_7_days_from_times(alos.as_ref(), tzais.as_ref());
        ...
        assert_almost_equal_datetime_option(
            ...
            &java_result,
            &format!(
                "get_tchilas_zman_kidush_levana_7_days_from_times regression test: \
                calendar date={:?}, lat={}, lon={}, elev={}, alos={:?}, tzais={:?}",
                date_time,
                30.407542238508753,
                35.866110022948135,
                63.435634057364425,
